...
# Load all Parquet files lazily
parquet_dir = "Data/Parquet"
df = pl.scan_parquet(os.path.join(parquet_dir, "*.parquet"))  # lazy dataframe

# Select only necessary columns
cols = ["Port", "Traffic", "Packets", "Bytes", "UniqueDests", "UniqueDest24s",
        "Lat", "Long", "ASN", "EventType"]
df = df.select(cols)

# Collect into Pandas only when necessary for sklearn
df_pd = df.collect().to_pandas()

X = df_pd.drop('EventType', axis=1)
Y = df_pd['EventType']
